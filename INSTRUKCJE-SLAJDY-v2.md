...
### Rozmiary tekstu (stałe px — slajd jest skalowany przez JS, więc NIE używaj `clamp`/`vw`)
| Element | Rozmiar | Waga |
|---------|---------|------|
| Tytuł główny (h1, slajd tytułowy) | `80px` (stały, dopuszczalne 78–82px) | 800 |
| Tytuł sekcji (h2, każdy slajd treści) | `72px` (stały) — **TEN SAM ROZMIAR NA WSZYSTKICH SLAJDACH** | 800 |
| Tekst treści | `28-32px` | 300–600 |
| Podtytuł / subtitle | `22-28px` | 300 |
| Etykiety / chipy | `0.75rem` – `0.85rem` | 600 |
| Eyebrow (nadtytuł) | `24px` | 600, uppercase, letter-spacing: 4px |

### Spójność tytułów (OBOWIĄZKOWE)
- **h1 / h2 musi mieć ten sam rozmiar na każdym slajdzie modułu** — `72px` dla slajdów treści, `80px` dla slajdu tytułowego.
- **NIE używaj `clamp()`, `vw`, `vh`** — slajd ma stałe wymiary 1920×1080 i jest skalowany skryptem JS. Responsywność jest realizowana przez skalowanie całej sceny, nie przez fluidne fonty.
- Jeśli tytuł nie mieści się w jednej linii, łam go `<br>` lub zwężaj `max-width` nagłówka — **nie zmniejszaj fontu**.
- `line-height: 1.2`, `letter-spacing: -1px`, `font-weight: 800`.
...
## 7. Tło i efekty wizualne

Dostępne są **dwa warianty tła** — wybierz jeden na slajd:

- **Wariant A (domyślny):** Animowane warstwy syntetyczne — `bg-grid` + `bg-glow` + `particles` + `corners`. Dobry do slajdów koncepcyjnych, diagramów, list.
- **Wariant B (foto):** Statyczne zdjęcie `bg-image` z ciemnym overlay + `corners`. Dobry do slajdów emocjonalnych, problemowych, sekcyjnych. **Bez** grid/glow/particles.

W obu wariantach **`corner--tl` i `corner--br` są obowiązkowe**.

---

### Wariant A — warstwy syntetyczne

### 7.1 Siatka (bg-grid) — ZAWSZE (wariant A)
```css
.bg-grid {
    position: absolute;
    inset: 0;
...
### Wariant B — tło ze zdjęciem (bg-image)

Alternatywne, statyczne tło wykorzystujące zdjęcie z katalogu `images/` z ciemnym overlay. **Stosuj zamiast** `bg-grid` + `bg-glow` + `particles` (te warstwy są wtedy pomijane). `corner--tl` i `corner--br` zostają.

### 7.6 Dostępne zdjęcia tła (wariant B)

Do wariantu B używaj **wyłącznie** dwóch plików z katalogu `images/` (ścieżka z poziomu slajdu zwykle `../../../images/`):

| Plik | Kompozycja | Wolna przestrzeń (na treść) | Kiedy używać |
|------|------------|-----------------------------|--------------|
| `background-5.jpg` | Gęsty plexus (sieć trójkątów z węzłami) zakotwiczony po **lewej stronie**, granat | Prawa połowa kadru | Slajdy z treścią po prawej / wyrównaną do prawej; layouty dwukolumnowe gdzie wizual jest po lewej |
| `background-6.jpg` | Plexus zakotwiczony przy **dolnej krawędzi**, gwiazdki rozproszone po całości, granat | Górna i środkowa część kadru | Slajdy z centralnym tytułem i treścią ułożoną wertykalnie (np. lista problemów jak `slides-02.html`) |

**Zasady:**
- Inne pliki (`background-1..4.jpg`) **nie są przeznaczone** do wariantu B w tym module.
- Wybór zdjęcia podporządkuj kompozycji treści — treść MUSI lądować w „wolnej" części kadru, nie nakładać się na gęsty plexus.
- Oba zdjęcia są w palecie cyan/granat — zachowują spójność z paletą `--cyan-*`.

### 7.7 Warstwa bg-image
```css
.bg-image {
    position: absolute;
    inset: 0;
    pointer-events: none;
    background:
        linear-gradient(rgba(10,14,23,0.66), rgba(10,14,23,0.76)),
        url('../../../images/background-5.jpg') center / cover no-repeat;
}
...
.bg-image::after {
    ...
    inset: 0;
    background:
        radial-gradient(ellipse at center, transparent 0%, rgba(3,7,18,0.16) 48%, rgba(3,7,18,0.56) 100%),
        linear-gradient(90deg, rgba(3,7,18,0.42) 0%, transparent 24%, transparent 76%, rgba(3,7,18,0.42) 100%);
}
```

Dwie warstwy overlay:
1. **Linear-gradient na samym `.bg-image`** — globalne ściemnienie (0.66 → 0.76), zapewnia czytelność tekstu.
2. **`::after` z radial + linear gradient** — winieta (przyciemnione rogi i krawędzie boczne) skupiająca wzrok na środku slajdu.

### 7.8 Markup wariantu B
```html
<div class="stage" id="stage">
    <!-- background -->
    <div class="bg-image"></div>

    <!-- corner accents (zachowane) -->
    ...
    <!-- logo + camera-box jak zwykle -->
</div>
```

**Uwaga:** w wariancie B **nie dodawaj** elementów `.bg-grid`, `.bg-glow`, `.particle` — zdjęcie zastępuje te warstwy.

### 7.9 Wybór wariantu
- **Wariant A (syntetyczny):** slajd tytułowy, agenda, diagramy, listy, definicje, podsumowania.
- **Wariant B (foto):** slajd otwierający sekcję, slajd problemowy („źródła problemów", „dlaczego to boli"), slajd emocjonalny / cytat.
- Nie mieszaj — jeden wariant na slajd.
...
4. **Dwa warianty tła** — A: animowana siatka + glow + particles, B: zdjęcie z overlay i winietą.
5. **Animowana siatka (wariant A)** — subtelna, powolna (20s).
6. **Pulsująca poświata (wariant A)** — centralny blur radialny.
7. **Cząsteczki unoszące się (wariant A)** — 6 małych kropek animowanych ciągle.
8. **Foto-tło (wariant B)** — `background-5.jpg` (plexus po lewej) lub `background-6.jpg` (plexus na dole) z dwoma overlayami (linear + radial winieta) dla czytelności.
9. **Narożnikowe L-kształty** — geometryczne akcenty w rogach (top-left, bottom-right) w obu wariantach.
10. **Kaskadowe animacje wejścia** — elementy pojawiają się sekwencyjnie.
11. **Brak zewnętrznych frameworków CSS** na slajdach — czysty CSS (Tailwind tylko w index).
12. **Stałe wymiary 1920×1080 + skalowanie JS** — fonty w `px` (nigdy `clamp`/`vw`); tytuły mają identyczny rozmiar na wszystkich slajdach modułu.
...
- [ ] `.stage` 1920x1080
- [ ] **Wybór wariantu tła:** A (syntetyczny) LUB B (zdjęcie) — nie mieszać
- [ ] Wariant A: `.bg-grid` + `.bg-glow` + 6× `.particle` z unikalnymi pozycjami
- [ ] Wariant B: `.bg-image` ze zdjęciem **`background-5.jpg`** (plexus po lewej) lub **`background-6.jpg`** (plexus na dole) + overlay + `::after` z winietą; treść w wolnej części kadru
- [ ] `.corner--tl` i `.corner--br` (w obu wariantach)
- [ ] `.content` z treścią slajdu
- [ ] Tytuł h1/h2 ma **stały rozmiar** zgodny z modułem (72px dla slajdów treści, 80px dla tytułowego)
- [ ] Brak `clamp()` / `vw` / `vh` w fontach
...
- [ ] Unikalne pozycje particles (różne od sąsiednich slajdów — dotyczy wariantu A)
